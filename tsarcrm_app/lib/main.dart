...
class _TableCard extends StatelessWidget {
  ...
  Future<bool> _confirmCloseOrder(
    BuildContext context,
    ApiOrder order,
  ) async {
    const paymentOptions = [
      ('card', 'Банковская карта', Icons.credit_card_rounded),
      ('cash', 'Наличные', Icons.payments_rounded),
    ];

    var selectedMethod = paymentOptions.first.$1;
  ...
  Future<void> _handlePrimaryAction(
    ...
    switch (table.status) {
      case 'occupied':
        if (activeOrder != null) {
          final confirmed = await _confirmCloseOrder(context, activeOrder);
                  ...
                  if (activeOrder != null) ...[
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.tonal(
                        ...
                        onPressed: () async {
                          final confirmed = await _confirmCloseOrder(
                            context,
                            activeOrder!,
                          );
                          if (confirmed) {
